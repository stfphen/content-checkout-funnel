 ...
 * - Tenant config stores only { design: { direction, overrides } }; tokens are
 *   resolved at render time so refining a direction here restyles every tenant
 *   on it, and switching direction never touches copy.
 */
...
export function resolveDesign(design) {
  ...
  return {
    ...
    heroVariant: HERO_VARIANTS.includes(direction.heroVariant) ? direction.heroVariant : "full-bleed",
    sectionOrder: resolveSectionOrder(direction.sectionOrder),
