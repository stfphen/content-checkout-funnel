...
// Manual (form/picker) patches may additionally address media slots — a
// deterministic path only; the AI schema still cannot emit media.
export const PATCHABLE_KEYS = [...EDITABLE_KEYS, "media"];
...
export function applyManualPatch(baseDraft, patch = {}) {
  ...
  // media is patchable but not an EDITABLE_KEY; carry it into the base first so
  // applyEditedSections's allowlist doesn't drop it.
  const baseWithMedia = merged.media ? { ...baseDraft, media: merged.media } : baseDraft;
  return applyEditedSections(baseWithMedia, merged);
}
