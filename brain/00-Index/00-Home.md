...
- **Deploy target:** `dgtlmag.com` on Hostinger VPS `62.72.16.32` — **LIVE at `main@814f861` (2026-07-02, smoke green: 5 funnels + admin + www/TLS + next/image hero)**; the `716e5b2` redeploy is the next ship — refreshed sequence in `docs/DEPLOY_NEXT.md` (needs migration 007, seed:tenants, `public/uploads` volume). See [[41-Deployment-Runbook]].
