...
- **Current priority:** merge the DMTV showcase branch, then the remaining ops/security items. See [[31-Current-Priorities]].
- **Active branch:** `claude/dmtv-website-redesign-7xlpzn` @ `52a86e6` — DMTV Studio showcase page via a generic tenant `template` registry (**292/292 tests + build green**, unmerged). Meanwhile `main` advanced to `b3d7e65`: YouTube hero polish (`7db08ab` — video-only display, aspect-aware cover, hidden-tab watchdog) merged and deployed; that deploy + polish are logged in **main's** copy of [[51-Timeline]] (not duplicated here — resolve as chronological union on merge).
