...
| ✅ **H4 — RESOLVED (07-04)** | **Unsubscribe amplification** (unauthenticated GET wrote NULL-tenant suppressions matching every team; side-effecting on GET). Fixed: HMAC-SHA256 signed tokens (`lib/unsubscribe.js`, new `UNSUBSCRIBE_SIGNING_SECRET`, fail-closed — no secret ⇒ no links emitted + all tokens rejected, 180-day expiry, `timingSafeEqual`); GET is now confirm-only, POST mutates; suppression is written with the token's `tenantId` (never NULL); send path finally embeds per-recipient links + RFC 8058 `List-Unsubscribe`/`List-Unsubscribe-Post` one-click headers (previously NO unsubscribe link existed in outgoing email at all); file-store suppression dedupe now tenant-scoped so one tenant's row can't swallow another's unsubscribe. Tests: `tests/unsubscribe.test.js`. (Supersedes M3.) | `app/api/unsubscribe/route.js`, `lib/unsubscribe.js`, `app/api/admin/outreach/queue/send/route.js`, `lib/store.js` | — |
...
| **L7 (NEW 07-04)** | Admin manual suppressions created with `tenantId=""` are global across **teams** (`outreach_suppression_list` has no `team_id` column; NULL-tenant rows match every team by design). Authenticated-only since the H4 fix, and acceptable in today's single-team reality — needs a `team_id` column for true multi-team isolation. | `app/api/admin/outreach/suppressions/route.js`, `lib/store.js` |
...
- ✅ **RESOLVED (07-04) — pg-vs-file-store dedupe parity:** the pg `createLead` branch turned out to already run `shouldSkipReliableDuplicate` and return `skippedDuplicate` (fixed earlier in `81e16b3`; this note was stale). Regression test added: `tests/lead-dedupe.test.js` (guards the shared helper; both branches use it). Residual (LOW): the pg dedupe is app-level read-then-insert — a concurrent double-submit can still race past it, and there is no DB unique constraint; acceptable at current volume.
...
- ✅ **RESOLVED (07-04) — outreach double-send race:** `updateOutreachQueueItem` gained an `expectedStatus` option (pg: `and status = $n` + rowCount check; file store: status check before write — null on mismatch); the send route now atomically claims `approved→sent` BEFORE calling Resend, so a double-submit's loser skips instead of re-sending. Failure still flips to `failed`. Tests: `tests/outreach-queue-claim.test.js`.
- ✅ **RESOLVED (07-04) — Stripe webhook idempotency:** new `processed_stripe_events(event_id PK)` ledger (`migrations/008_processed_stripe_events.sql` + `ensureSchema` + file-store key); the webhook claims `event.id` insert-or-ignore BEFORE fulfilling (`markStripeEventProcessed`), a thrown fulfillment releases the claim (`unmarkStripeEventProcessed`) and returns 500, and `leadMissing` now returns 500 so Stripe retries instead of losing the event; the `alreadyPaid` metadata guard stays as defense-in-depth. Residual (LOW): no idempotency key on session create. Tests: `tests/stripe-checkout.test.js` (ledger cases).
...
- **Missing indexes (MED):** `outreach_*` and `prospecting_batches` have PK-only indexes but are queried by `tenant_id` join + `created_at/updated_at` order; `leads`/`calls` tenant-only filters can't use the team-leading composite indexes; audit-log team filter is an unindexable `metadata->>'teamId'` expression. Concrete `CREATE INDEX` list in the audit report — propose as migration `009_performance_indexes.sql` (007 = media_assets, 008 = processed_stripe_events).
...
Code-side C2/H2/M1/M2/L1, login-H1, **H4, createLead dedupe parity, outreach double-send, and
Stripe webhook idempotency** are **done** (07-04 sweep on `claude/review-calendar-booking-prompt-qgovyz`).
Remaining, in order:
1. **Ops:** rotate the four provider keys + set a strong prod DB password (C1, H3) + set
   `UNSUBSCRIBE_SIGNING_SECRET` in prod (`openssl rand -base64 32`) so unsubscribe links go out.
2. Extend rate limiting to the remaining public POSTs; add performance-indexes migration (next free number: 009).
3. File-store write mutex; batch-import idempotency.
