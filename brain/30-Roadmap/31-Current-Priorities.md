...
1. **Repo recovery & branch cleanup.** ~15+ local branches + backups + wip/rescue + remotes create
   confusion. Consolidate to one clean deployable line. **Never delete/force-push without explicit
   confirmation** ([[47-Git-Workflow]]).
2. **Get the live site green & current.** `dgtlmag.com` history includes a 502 (fixed) and a VPS
